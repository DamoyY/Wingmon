...
type FindArgs = { query: string; tabId: number };

type PageContentItem = { pageNumber: number; content: string };
...
const resolvePageContentItem = (
  ...
  return { content: item.content, pageNumber };
};
...
const buildFindPageResults = (
  ...
  return sortedPages.reduce<FindToolPageResult[]>((result, page) => {
    ...
    result.push({ lines, pageNumber: page.pageNumber });
    return result;
  }, []);
};
...
const parameters = {
    ...
    properties: {
      ...
      tabId: { description: t("toolParamTabId"), minimum: 1, type: "integer" },
    },
