...
export type RequestBodyBuilder<TBody> = (
  settings: Settings,
  systemPrompt: string,
...
export type RequestBodyBuilders<TBody, TFallbackBody> = {
  buildStreamRequestBody: RequestBodyBuilder<TBody>;
  buildNonStreamRequestBody: RequestBodyBuilder<TFallbackBody>;
};

export type BaseApiStrategy<TStream, TResponse, TBody, TFallbackBody> =
  RequestBodyBuilders<TBody, TFallbackBody> & {
    stream: (
