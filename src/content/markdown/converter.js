...
const getLabelFromIds = (idMap, ids) => {
...
const getButtonLabel = (idMap, button) => {
  const directText =
...
const replaceButtons = (root) => {
  const idMap = buildIdMap(root);
  const buttons = root.querySelectorAll("[data-llm-id]");
...
const convertPageContentToMarkdown = (pageData) => {
  ...
  const viewportMarker = bodyClone.querySelector("[data-llm-viewport-center]");
  if (!viewportMarker) {
    throw new Error("未找到视口中心标记，无法定位截取范围");
  }
  viewportMarker.textContent = viewportMarkerToken;
  const content = turndown.turndown(bodyClone.innerHTML);
