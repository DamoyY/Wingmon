...
const persistConversation = async () => {
  ...
  handleCopyMessage = async (indices) => {
    const normalized = normalizeIndices(indices),
      text = resolveCombinedContent(normalized);
    if (typeof navigator?.clipboard?.writeText !== "function") {
      throw new Error("当前环境不支持复制");
    }
    await navigator.clipboard.writeText(text);
  },
  collectHiddenIndicesForward = (startIndex) => {
    const hiddenIndices = [];
  ...
  collectHiddenIndicesBackward = (startIndex) => {
    const hiddenIndices = [];
  ...
  handleDeleteMessage = async (indices, refreshMessages, animateRemoval) => {
    if (state.sending) {
  ...
  createMessageActionHandlers = (refreshMessages, animateRemoval) => {
