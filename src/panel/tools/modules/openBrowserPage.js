...
const parameters = {
  ...
  buildOpenPageAlreadyExistsOutput = (tabId) =>
    t("statusAlreadyExists", [String(tabId)]),
  buildOpenPageReadOutput = ({ title, tabId, content = "", isInternal }) =>
    buildPageReadResult({
      headerLines: [
        t("statusOpenSuccess"),
        `${t("statusTitle")}: "${title}"`,
  ...
  execute = async ({ url, focus }) => {
    const followMode = await shouldFollowMode(),
      ...
      normalizedTabs = tabs.map(normalizeTab).filter(Boolean),
    ...
    const {
      ...
      url: pageUrl,
      content,
    } = await fetchPageMarkdownData(tab.id);
    return buildOpenPageReadOutput({
      title,
      tabId: tab.id,
      content,
