...
const requiredToolAdapterKeys = [
  ...
  createApiStrategies = (toolAdapter) => ({
    chat: {
  ...
  getApiStrategy = (apiType, toolAdapter) => {
    const resolvedAdapter = ensureToolAdapter(toolAdapter),
      apiStrategies = createApiStrategies(resolvedAdapter),
      strategy = apiStrategies[apiType];
    if (!strategy) {
